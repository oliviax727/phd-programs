...
class SimulationReformatter:
    ...
    @staticmethod
    def calculate_observation_time_from_date(
        phase_ref_point=omath.ZENITH_530,
    ...
    def generate_dynamic_settings(
        ...
        ref_time, _ = SimulationReformatter.calculate_observation_time_from_date(
        ...
        decc = omath.diff(
            omath.circmean(dcs[-1, :, :], (-180, 180)),
            omath.circmean(dcs[0, :, :], (-180, 180)),
        )
    ...
    def generate_osm_from_simulation(
        ...
        if regrid_flag:
            values, voxels = SimulationReformatter.regrid_datacube(
                values=values, voxels=voxels, max_freq_res=max_freq_res
            )
        else:
        ...
        if osm_output != "":
            SimulationReformatter.save_datacube_to_osm(
                values=values,
                voxels=voxels,
                f_ref=f_ref,
                phase_ref_point=phase_ref_point,
    ...
    def convert_osm_file_to_arrays(
        ...
        output_data["cumulative_voxels"] = (
            np.array(df["RA"]).reshape(d),
            np.array(df["Dec"]).reshape(d),
            np.array(df["Freq0"]).reshape(d),
        )
