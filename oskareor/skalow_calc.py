...
class SKAMath:
    ...
    def circmean_deg(angles: float, bounds: tuple[float] = (0, 360)) -> float:
        ...
        return np.rad2deg(
            circmean_radians(
                np.deg2rad(angles),
                high=np.deg2rad(bounds[1]),
                low=np.deg2rad(bounds[0]),
            )
        )
    ...
    def sinc(x: float, f: float = 1, ph: float = 0, amp: float = 1) -> float:
        ...
        return amp * np.nan_to_num(
            np.sin(2 * np.pi * f * x + ph) / (2 * np.pi * f * x + ph),
            nan=1,
            posinf=1,
            neginf=1,
        )
...
class OSKARFileConfig:
    ...
    def dir_list_sorted(dir_: str) -> str:
        ...
        sorted_files, _ = zip(
            *np.sort(
                np.array(
                    list(map(lambda x: (x, int(x.split(".")[1].split("_")[0])), files)),
                    dtype=sort_type,
                ),
                order="num",
            )
